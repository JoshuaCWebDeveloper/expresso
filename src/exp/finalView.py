...
class Ui_Form(QtGui.QWidget):
	...
	self.fillList()
	self.pushButtonGenerate.clicked.connect(self.findTicked)
    ...
    #Find the ticked elements in list and add it

    def findTicked(self):
	self.fileHandle=h5py.File(root+'/exp/data/'+self.lineEditName.text().__str__()+'.hdf5','w')

	for i in range(self.listWidgetNetList.count()):
	    if self.listWidgetNetList.item(i).checkState()!=0:
		print self.listWidgetNetList.item(i).text()
		self.tempHandler=self.protoHandler.__deepcopy__()
		self.temp=self.tempHandler.__deepcopy__()
		self.fillNet(self.temp,self.listWidgetNetList.item(i).text().__str__())
		open(root+'/net/temp/temp.prototxt','w').write(self.tempHandler.__str__())
		#Change Net everytime
		channel_swap=(2,1,0) if self.netHandler.net[self.index].channel_swap else None
		self.net=caffe.Classifier(str(root+'/net/temp/temp.prototxt'),str(self.netHandler.net[self.index].modelpath),mean=np.load(str(self.netHandler.net[self.index].meanpath)),gpu=False,raw_scale=int(self.netHandler.net[self.index].raw_scale),channel_swap=channel_swap)
		...
		self.fileHandle.create_dataset(self.listWidgetNetList.item(i).text().__str__(),data=data,**comp_kwargs)
